...
class ResolutionStack {
    Vector<TermToBeResolved> vec;
    
	ResolutionStack() {
        vec = new Vector<TermToBeResolved>(10, 3);
    }
    ...
    public void push(TermToBeResolved trmRes) {
        vec.add(trmRes);
    }
    
    public TermToBeResolved lastElement() {
    		return vec.get(vec.size()-1);
    }

    public TermToBeResolved pop() {
        TermToBeResolved termRes = (TermToBeResolved) lastElement();
        vec.remove(vec.size() - 1);

        return termRes;
    }

    public TermToBeResolved getTop() {
        return (TermToBeResolved) lastElement();
    }
	...
	public TermToBeResolved get(int i) {
		return vec.get(i);
	}
}
