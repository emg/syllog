...
public class RuleVector {    
    Vector<PrologRule> vec;

	RuleVector() {
        vec = new Vector<PrologRule>(3);
    }

    public PrologRule getAt(int i) {
        return vec.get(i);
    }
    

    public PrologRule get(int i) {
        return vec.get(i);
    }
    ...
    public void addRule(PrologRule regle) {
        vec.addElement(regle);
    }
	...
	public void add(int i, PrologRule pRule) {
		vec.add(i, pRule);
	}

	public void remove(int indRegle) {
		vec.remove(indRegle);
	}
