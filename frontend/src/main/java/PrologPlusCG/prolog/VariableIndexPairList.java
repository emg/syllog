...
// VariabelIndexPairList is a list of 
// VariableIndexPair objects
// with a member: PrologDataIndexPair ValInd.
class VariableIndexPairList {
	private Vector<VariableIndexPair> vec; 
	PrologDataIndexPair ValInd;
	
	VariableIndexPairList() {
		vec = new Vector<VariableIndexPair>(5, 2);
		ValInd = null;
	}
	...
	public VariableIndexPair get(int index) {
		if (index < vec.size()) {
	...
	public void add(VariableIndexPair vip) {
		vec.add(vip);
	}
	
	public void addElement(VariableIndexPair vip) {
		add(vip);
	}
